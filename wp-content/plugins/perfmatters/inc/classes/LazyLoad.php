...
class LazyLoad
{
	...
	private static function lazyload_parent_leading_exclusions($html, &$buffer) {
        ...
        if(!empty($parent_exclusions)) {
            ...
            $clean_dom->loadHTML($buffer, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOERROR | LIBXML_ERR_NONE);
            ...
            if(!is_null($elements)) {
            	...
            	$dom = new \DOMDocument();
                $dom->loadHTML($html, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOERROR | LIBXML_ERR_NONE);
